...
Page({
  ...
  data: {
    ...
    answerUrl: '1.0?method=pingshifen.question.hand_paper&course_id=' + wx.getStorageSync('pingshifen_current_course_id') ,//交卷URL
    answers:{
  ...
  setSubmit:function(){
    ...
    .then((data) =>{
      if(data.data.status == 1){
        wx.redirectTo({
          url: `/pages/app/answer/answer_mark/mark?subject=${this.data.subject}&type=${this.data.type}&time=${this.params.seconds}&mark=${data.data.data.score}&mid=${data.data.data.mid}&error=${this.data.answers.error}`
        })
      }
    })
  },
